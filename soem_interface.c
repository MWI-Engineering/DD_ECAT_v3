// soem_interface.c - Fixed version using existing PDO configuration
...
// These match the exact PDO mapping from the device
typedef struct PACKED
{
    uint16 controlword;         // 0x6040:0x00 - [0x0000.0]
    int8   modes_of_operation;  // 0x6060:0x00 - [0x0002.0]
    int16  target_torque;       // 0x6071:0x00 - [0x0003.0]
    int32  target_position;     // 0x607A:0x00 - [0x0005.0]
    int32  target_velocity;     // 0x60FF:0x00 - [0x0009.0]
    int16  torque_offset;       // 0x60B2:0x00 - [0x000D.0]
    uint32 tuning_command;      // 0x2701:0x00 - [0x000F.0]
    uint32 physical_outputs;    // 0x60FE:0x01 - [0x0013.0]
    uint32 bit_mask;            // 0x60FE:0x02 - [0x0017.0]
    uint32 user_mosi;           // 0x2703:0x00 - [0x001B.0]
    int32  velocity_offset;     // 0x60B1:0x00 - [0x001F.0]
} SomanetOutputs_t;

typedef struct PACKED
{
    uint16 statusword;              // 0x6041:0x00 - [0x0023.0]
    int8   modes_of_operation_disp; // 0x6061:0x00 - [0x0025.0]
    int32  position_actual_value;   // 0x6064:0x00 - [0x0026.0]
    int32  velocity_actual_value;   // 0x606C:0x00 - [0x002A.0]
    int16  torque_actual_value;     // 0x6077:0x00 - [0x002E.0]
    uint16 analog_input_1;          // 0x2401:0x00 - [0x0030.0]
    uint16 analog_input_2;          // 0x2402:0x00 - [0x0032.0]
    uint16 analog_input_3;          // 0x2403:0x00 - [0x0034.0]
    uint16 analog_input_4;          // 0x2404:0x00 - [0x0036.0]
    uint32 tuning_status;           // 0x2702:0x00 - [0x0038.0]
    uint32 digital_inputs;          // 0x60FD:0x00 - [0x003C.0]
    uint32 user_miso;               // 0x2704:0x00 - [0x0040.0]
    uint32 timestamp;               // 0x20F0:0x00 - [0x0044.0]
    int32  position_demand_internal; // 0x60FC:0x00 - [0x0048.0]
    int32  velocity_demand_value;   // 0x606B:0x00 - [0x004C.0]
    int16  torque_demand;           // 0x6074:0x00 - [0x0050.0]
} SomanetInputs_t;
    ...
    if (ec_init(ifname)) {
        ...
        if (ec_config_init(FALSE) > 0) {
            ...
            if (ec_slave[1].Obytes != sizeof(SomanetOutputs_t) || 
                ...
                printf("SOEM_Interface: Warning - PDO size mismatch!\n");
                printf("  Expected: Output=%zu Input=%zu\n", sizeof(SomanetOutputs_t), sizeof(SomanetInputs_t));
                printf("  Actual:   Output=%d Input=%d\n", ec_slave[1].Obytes, ec_slave[1].Ibytes);
                // Don't return error - the sizes from device are authoritative
            }
            ...
            int wait_count = 0;
            const int max_waits = 50; // wait up to 5 seconds (50 * 100ms)
            ...
            printf("SOEM_Interface: outputsWKC: %d, inputsWKC: %d\n", 
            ...
            if (wkc >= expectedWKC && somanet_inputs) {
                printf("SOEM_Interface: Initial drive state:\n");
                ...
                printf("SOEM_Interface: Initial communication check - WKC: %d, expected: %d\n", wkc, expectedWKC);
            }
            ...
            for (int i = 0; i < 3; i++) {
                ...
                if (wkc >= expectedWKC && somanet_inputs) {
                    ...
                    printf("SOEM_Interface: Communication issue during state transition %d (WKC: %d)\n", i, wkc);
                }
            ...
            for (int i = 1; i <= ec_slavecount; i++) {
                ...
                if (ec_slave[i].state != EC_STATE_OPERATIONAL) {
                    printf("SOEM_Interface: Slave %d not operational. State: 0x%02X\n", i, ec_slave[i].state);
                    all_operational = 0;
                }
            }
            ...
            if (all_operational) {
                ...
                // Don't immediately fail - some drives may work in SAFE_OP
                printf("SOEM_Interface: Continuing with current state...\n");
                
                // Start EtherCAT communication thread anyway
                ecat_thread_running = 1;
                if (pthread_create(&ecat_thread, NULL, ecat_loop, NULL) != 0) {
                    fprintf(stderr, "SOEM_Interface: Failed to create EtherCAT thread.\n");
                    ec_close();
                ...
                master_initialized = 1;
                printf("SOEM_Interface: EtherCAT master initialized (some slaves may not be operational).\n");
                return 0;
            }
        } else {
